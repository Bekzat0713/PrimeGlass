...
assert.match(stylesheet, /\.reels-carousel::before,\.reels-carousel::after/, 'Reels marquee needs browser-safe soft edge fading');
assert.match(stylesheet, /\.reveal,\.reveal\.is-visible\{opacity:1;transform:none;transition:none\}/, 'Site content must remain visible without observer animations');
...
assert.match(renderHome(), /class="hero-scroll-cue"[^>]*href="#services"[^>]*><span>Смотреть услуги<\/span>/, 'Hero scroll cue must lead directly to services');
...
assert.match(renderHome(), /id="services"[\s\S]*id="reels"/, 'Services catalog must appear before the Instagram section');
